...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class SetupCompleteScreen : UserControl, IScreen
    {
        ...
        private ServiceController m_PTPSyncServiceController;
        ...
        {
            InitializeComponent();
            InitializePTPSyncServiceController();
            ...
            get
            {
                ...
                if (m_state != null)
                {
                    try
                    {
                        ...
                        try
                        {
                            ConfigurationFile PTPSyncConfig = ConfigurationFile.Open("PTPSync.exe.config");

                            // Some of the crypto settings elements were renamed for consistency, remove the old ones
                            CategorizedSettingsElementCollection cryptoSection = PTPSyncConfig.Settings["cryptographyServices"];
                            cryptoSection.Remove("RetryDelayInterval");
                            cryptoSection.Remove("MaximumRetryAttempts");

                            // Example connection settings are updated between builds - remove the section and PTPSync will add back the latest
                            PTPSyncConfig.Settings.Remove("exampleConnectionSettings");

                            // Data publisher categories are now always lower case (such that code references are case insensitive)
                            PTPSyncConfig.Settings.Remove("dataPublisher");

                            PTPSyncConfig.Save(ConfigurationSaveMode.Full);
                        }
                        catch
                        {
                        ...
                        // If the user requested it, start or restart the PTPSync service
                        if (m_serviceStartCheckBox.IsChecked.Value)
                        {
                            try
                            {
#if DEBUG
                                Process.Start("PTPSync.exe");
#else
                                m_PTPSyncServiceController.Start();
#endif
                            }
                            catch
                            {
                                MessageBox.Show("The configuration utility was unable to start PTPSync service, you will need to manually start the service.", "Cannot Start Windows Service", MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }

                        // If the user requested it, start the PTPSync Manager.
                        if (m_managerStartCheckBox.IsChecked.Value)
                        {
                            Process.Start("PTPSyncManager.exe");
                        }
                    }
                    finally
                    {
                        if (m_PTPSyncServiceController != null)
                            m_PTPSyncServiceController.Close();
                    }
                }
        ...
        public Dictionary<string, object> State
        {
            ...
            set
            {
                ...
                if (Convert.ToBoolean(m_state["restarting"]))
                    m_serviceStartCheckBox.Content = "Restart the PTPSync";
            }
        }
        ...
        // Initializes the PTPSync service controller.
        private void InitializePTPSyncServiceController()
        {
            ServiceController[] services = ServiceController.GetServices();
            m_PTPSyncServiceController = services.SingleOrDefault(svc => string.Compare(svc.ServiceName, "PTPSync", true) == 0);
        }

        // Initializes the state of the PTPSync service checkbox.
        private void InitializeServiceCheckboxState()
        {
#if DEBUG
            bool serviceInstalled = File.Exists("PTPSync.exe");
#else
            bool serviceInstalled = m_PTPSyncServiceController != null;
#endif
            m_serviceStartCheckBox.IsChecked = serviceInstalled;
            m_serviceStartCheckBox.IsEnabled = serviceInstalled;
        }

        // Initializes the state of the PTPSync Manager checkbox.
        private void InitializeManagerCheckboxState()
        {
            bool managerInstalled = File.Exists("PTPSyncManager.exe");
        ...
        {
            string configFileName = Path.Combine(Directory.GetCurrentDirectory(), "PTPSync.exe.config");
            ...
            {
                string archiveBinding = string.Format(GrafanaArchiveBinding, "stat", "http.rest://localhost:6357/api/grafana");
            ...
            if (setupHistorian)
            {
                ...
                {
                    string archiveBinding = string.Format(GrafanaArchiveBinding, historianAcronym, "http.rest://localhost:6457/api/grafana");
        ...
        {
            string configFile = Directory.GetCurrentDirectory() + "\\PTPSync.exe.config";
