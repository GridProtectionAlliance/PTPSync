...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class NodeSelectionScreen : UserControl, IScreen
    {
        ...
        private void UpdateDataGrid()
        {
            ...
            {
                m_infoTextBlock.Text = "Please select the node you would like the PTPSync to use.";
            }
            else
            {
            ...
            if (applyChangesToService)
                nodeId = GetNodeIdFromConfigFile("PTPSync.exe.config");
            else if (applyChangesToLocalManager)
                nodeId = GetNodeIdFromConfigFile("PTPSyncManager.exe.config");
            else
                nodeId = Guid.Empty;
            ...
            if (applyChangesToService)
                TryAddNodeInfo("PTPSync.exe.config");

            if (applyChangesToLocalManager)
                TryAddNodeInfo("PTPSyncManager.exe.config");
        ...
        private IDbConnection GetConnectionFromConfigFile()
        {
            ...
            string configFileName = Directory.GetCurrentDirectory() + "\\PTPSync.exe.config";
        ...
        {
            return GetValueOfSystemSetting("PTPSync.exe.config", "CompanyName");
        }
        ...
        // Occurs when the user selects a different node to be used by the PTPSync.
