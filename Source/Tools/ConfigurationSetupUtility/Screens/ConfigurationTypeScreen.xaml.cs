...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ConfigurationTypeScreen : UserControl, IScreen
    {
        ...
        public ConfigurationTypeScreen()
        {
            ...
            try
            {
                // Determine if the PTPSync is installed in this folder (could be stand-alone debug application or service), we check exe
                // instead of config file since config file can remain between installations. Since only the PTPSync can be setup with a
                // XML or web service configuration, we disable these radio buttons for PTPSync Manager only installations.
                string exeFileName = Directory.GetCurrentDirectory() + "\\PTPSync.exe";

                if (!File.Exists(exeFileName))
                {
                    // If the PTPSync is not be installed user may have chosen to only install the PTPSync Manager on this system,
            ...
            {
                // Not failing if we cannot determine if PTPSync is available...
            }
        }
