...
namespace ConfigurationSetupUtility
{
    ...
    public partial class App : Application
    {
        ...
        public const string ApplicationExe = "PTPSync.exe";
        public const string ApplicationConfig = "PTPSync.exe.config";
        public const string Manager = "PTPSyncManager";
        public const string ManagerExe = "PTPSyncManager.exe";
        public const string ManagerConfig = "PTPSyncManager.exe.config";
        public const string BaseSqliteConfig = "PTPSync.db";
        public readonly static string SqliteConfigv2 = "PTPSync" + DatabaseVersionSuffix + ".db";
        public const string SqliteSampleData = "PTPSync-SampleDataSet.db";
        public const string SqliteInitialData = "PTPSync-InitialDataSet.db";
        ...
        public App()
        {
            ...
            // Attempt to create an event log source for the PTPSync Manager for authentication logging. This needs to be done
            // here since the CSU runs with administrative privileges and the PTPSync Manager normally does not; also there is
            // a short system delay that exists before you can write to a new event log source after it is first created.
            try
            {
                string applicationName = "PTPSync";

                // Create the event log source based on defined application name for PTPSync if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");

                applicationName = "PTPSync Manager";

                // Create the event log source based on defined application name for PTPSync Manager if it does not already exist
                if (!EventLog.SourceExists(applicationName))
                    EventLog.CreateEventSource(applicationName, "Application");
            }
            catch (Exception ex)
            {
                m_errorLogger.Log(new InvalidOperationException(string.Format("Warning: failed to create or validate the event log source for the PTPSync Manager: {0}", ex.Message), ex), false);
            }
        }
