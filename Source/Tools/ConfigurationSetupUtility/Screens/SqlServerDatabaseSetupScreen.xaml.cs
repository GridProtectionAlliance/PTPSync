    ...
    public partial class SqlServerDatabaseSetupScreen : UserControl, IScreen
    {
            ...
            get
            {
                ...
                else
                {
                    ...
                    if (!hostIsLocal && m_createNewUserCheckBox.IsChecked != true && m_checkBoxIntegratedSecurity.IsChecked == true)
                    {
                        ...
                        {
                            const string failMessage = "Configuration Setup Utility has detected that the PTPSync service account ({0}) is a local user, " +
                                "but the database server is not local. This user will not be able to log into the database using integrated security. " +
                                "Please either change the account under which the PTPSync service runs, or choose to create a new database user.";
        ...
        private void InitializeState()
        {
            if (m_state != null)
            {
                ...
                m_databaseNameTextBox.Text = migrate ? "PTPSync" + App.DatabaseVersionSuffix : "PTPSync";
